...
s <- read.table("schedule.tsv", sep = "\t", header = T, na.strings=c("","NA")) %>%
    ...
    # style topic
    dplyr::mutate(topic = ifelse(tolower(topic) == "break" | tolower(topic) == "lunch" | tolower(topic) == "fika", paste0("<span style='color:white;border-radius:4px;padding:1px;background:#BDBDBD;'><span class='topic'>", topic, "</span></span>"), topic)) %>%
    dplyr::mutate(topic = ifelse(grepl("dinner",tolower(topic)), paste0("<span style='color:white;border-radius:4px;padding:1px;background:#BDBDBD;'><span class='topic-dinner'>", topic, "</span></span>"), topic)) %>%
    dplyr::mutate(topic = ifelse(!startsWith(topic, "<span"), paste0("<span style='font-size:110%;font-weight:bold;margin-right:5px;'>", topic, "</span>"), topic)) %>%
    # add links to topic
...
if (all(is.na(s$assistant)) | all(s$assistant == "")) {
    ...
    cnames <- c("Time", "Topic", "Instructor", "Assistants")
}
```
...
if(!is.null(asst)) {
  cat('<p style="margin-top:20px;">',
           '<span style="margin-left:4px; margin-right:4px;text-align:center;"><i class="fas fa-users" style="margin-left:4px; margin-right:4px;"></i></span>',
           paste(asst,collapse=", "),
           '</p>')
}
```
