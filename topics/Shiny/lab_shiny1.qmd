...
To understand the concepts of `input`, `output`, `ui` and `server` variables, look at the image below. 

![](assets/shiny_anatomy.png){width="80%" fig-align="center"}

 * `ui` - defines the user interface of the app. It is a collection of widgets that are displayed on the webpage. It is defined using shiny functions like `sliderInput()`, `plotOutput()`, etc. Here, you see that the usewr-interface has both `input` and `output` widgets.
 * `server` - defines the server logic of the app where all the calculations are performed. It is a collection of instructions that are executed when the app is run. It is defined using R code and uses `input` and `output` variables to access the values of input widgets and write content to output widgets respectively with functions like `renderPlot()`, `renderText()`, etc.
 * `input` - These are the variables recieved from the client and the values are sent to the server for processing. 
 * `output` - These are the variables sent from the server to the client and the values are displayed on the `UI` part of webpage.

You can think of `ui` as the front-end of the app and `server` as the back-end of the app. The `ui` and `server` are connected together using `shinyApp()` function which launches the app.
