...
::: {.callout-note}
In these exercises you will learn how to embed dynamic components, such as interactive plots and widgets, directly into a Quarto document. 
...
- Add popup information with the island names.
...
- Add legend to the map to label the three islands.

::: {.callout-tip}
## Extras
In addition to main `leaflet` package there is also `leaflet.extras` package that provides additional functionality to the leaflet maps. This package is not part of this workshop, but if you want to explore it, you can find more information about it [here](https://cran.r-project.org/web/packages/leaflet.extras/index.html).
:::
...
plot_ly(data = penguins, x = ~flipper_length_mm, y = ~body_mass_g, 
        color = ~species, 
        colors = c("darkorange","darkorchid","cyan4"), size=2)
```
````
...
plot_ly(data = penguins, x = ~flipper_length_mm, y = ~body_mass_g, 
        color = ~species, 
        colors = c("darkorange","darkorchid","cyan4"), size=2, type = "scatter", mode = "markers")
```
````
...
`plot_ly` allows us to plot different plot types, for example, histograms, boxplots, scatterplots... using argument `type = ~ ...`. Example below shows how to plot boxplot and scatterplot and how to combine these two plots together using the `subplot` command.

````{.r}
```{{r}}
p1 <- plot_ly(data = penguins, x = ~island, y = ~flipper_length_mm, 
        color = ~island, type = "box", 
        colors = c("#10a53dFF","#541352FF","#ffcf20FF"), size=2)
...
- Explore the relationship between REM sleep and total sleep across different dietary categories of mammals.
...
With `plotly` package you can also build heatmaps, which are commonly used to visualize gene expression data. You can build heatmaps specifying `type = "heatmap"` in the `plot_ly()` function. As the input for the heatmap you need to provide a square matrix. 

- Generate a random matrix with 10 rows (as genes) and 10 columns (as samples) using `matrix()` function.
- Use `plot_ly()` function to generate a heatmap.
- Customize the heatmap by adding a title, adjusting the color scale, and adding axis labels.
...
Let's show how to create a dynamic plot using the BCG immunization coverage dataset. This dataset contains information about the share of newborns immunized against tuberculosis (TB) in different countries over time. We want to focus on countries that start with the letter "B".
...
ggplot(tb_data_filt, aes(x = Year, y = Share_of_newborns, color = Entity)) +
    ...
    scale_color_manual(values = c("#004488", "#4477AA", 
    "#EE6677", "#228833", "#CCBB44", "#997700", 
    "#CC3311", "#009988", "#66CCEE", "#AA3377", 
    "#BBBBBB", "#6699CC", "#CCDDAA")) +
    theme_bw() +
...
`ggiraph` becomes very powerful when multiple plots are combined together. [Here](https://r-graph-gallery.com/414-map-multiple-charts-in-ggiraph.html) you can check how to combine different plot types together. 
...
- Now try to combine two different plot types together.
- Create a boxplot to visualize the total amount of total sleep across different diets and a scatterplot to visualize the relationship between REM sleep and total sleep across different dietary categories of mammals. Combine these two plots together using `patchwork` package. 
- Try out different tooltip options for both plots, for example, for boxplot you can use `tooltip = vore` and for scatterplot `tooltip = name`.
- Add hover effect to both plots, so that when you hover over a point in one plot, the corresponding point in the other plot is highlighted.

# Time series with dygraphs

The `dygraphs` package provides a rich set of tools for creating dynamic time series visualizations. The main function is `dygraph()`, which takes a time series object as input and produces a dynamic plot.

The example below shows a basic dygraph built from a multiple time series object. The graph is fully dynamic: moving the mouse over the lines reveals individual values, and you can zoom in by selecting a region of the plot (double-click to zoom out).
...
- In this exercise you will use [US economic time series](https://ggplot2.tidyverse.org/reference/economics.html) dataset that is part of `ggplot2()` package.
- First check the dataset and familiarise yourself with data structure.
- Start exploring the dataset and plot different time series using `dygraph()`. For example, you can plot the number of unemployed people over time.
- Try different options to customize the plot, for example [series options](https://rstudio.github.io/dygraphs/gallery-series-options.html), [range selector](https://rstudio.github.io/dygraphs/gallery-range-selector.html), [axis options](https://rstudio.github.io/dygraphs/gallery-axis-options.html) and more. 
...
```{{r}}
datatable(msleep, options = list(pageLength = 5), 
    caption="Table 1: Mammals sleep dataset") %>%
...
- Display the first 10 rows of `counts_raw.txt` and `metadata.csv`. Once tables are rendered and displayed play around with the 
...
The `reactable` package is another powerful tool for creating dynamic data tables in R. It provides a flexible and customizable way to display tabular data with features like sorting, filtering, pagination, and more. The main function in this package is `reactable()`.

## Exercise:
- Use the same files as in the previous exercise, `counts_raw.txt` and `metadata.csv`.
- Similar to the task in previous exercise, now explore creating tables using `reactable()`.
...
1. Create a `SharedData` object
2. Make `htmlwidgets` with `SharedData` input
    - Before these were two `ggplotly` objects
    - Now, we will make one `ggplotly` and one `datatable`
3. Output results
...
# compose output
bscols(dt_penguins, lf_stations, widths = c(6, 6))
```
````
...
ObservableJS is an approach that allows dynamic features to be included in a Quarto document. It is an entirely separate language outside of R that uses JavaScript and allows excellent functionality similar to what is provided by a Shiny Server.
...
Now we have loaded the external CSV file into the Observable environment.
<br>
