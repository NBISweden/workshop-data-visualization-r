...
:::{.callout-tip}
To specify manual colors, you could specify by their `names` or their `hexadecimal codes`. For example, you can choose the colors based on `names` from an online source like in this [cheatsheet](https://r-charts.com/colors/) or you can use the `hexadecimal code` and choose it from a source like [here](https://htmlcolorcodes.com/). I personally prefer the `hexa` based options for manual colors.
:::
...
Now that we have summarized the data to be able to plot the bar graph that we want, we can just input the data to ggplot as well using the `%>%` sign. 
...
:::{.callout-note}
It is important that you keep track of what kind of aesthetics you give when you initialize `ggplot()` and what you add in the `geoms()` later. 
:::
