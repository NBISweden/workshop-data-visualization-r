---
title: "Contents"
description: "The information about the course topics and the materials related to each topic can be found here"
date: ""
toc: false
number-sections: false
sidebar: false
format: html
---

::: {.callout-note} 

The contents of this course are currently being generated. It will be updated here regularly until the course is completed. 

:::

```{r}
#| echo: false
#| eval: true


library(dplyr)
library(kableExtra)
library(fontawesome)
library(htmlTable)

s <- read.table("./contents.csv", sep = ',', header = 1)
r <- rle(s$Main)
row.names(s) <- NULL
cnames <- c('Topic','Lecture','Tutorial')
color_group <- c()
s %>%
    dplyr::mutate(Lecture = ifelse(Lecture=="", Lecture, paste0("<span><a class='marker' href='", Lecture, "'>", fa("fas fa-file-powerpoint"), "</a></span>"))) %>%
    dplyr::mutate(Tutorial = ifelse(Tutorial=="", Tutorial, paste0("<span><a class='marker' href='", Tutorial, "'>", fa("fas fa-file-lines"), "</a></span>"))) %>%
    dplyr::select(-Main) %>%
    setNames(cnames) %>%
    addHtmlTableStyle(align = "lcc", align.header = "ccc", css.rgroup = "color:#4d6804;font-weight:800;", css.cell = "padding-right:1em;") %>%
    htmlTable(rnames = FALSE, rgroup = r$values, n.rgroup = r$lengths)
```


<!-- marker legend -->
<p class="small" style="margin-top:20px;text-align:center;">
<span>
<span style="margin-left:6px; margin-right:6px;">
{{< fa file-powerpoint >}}  Slides
</span>
<span style="margin-left:6px; margin-right:6px;">
{{< fa file-lines >}}  Lab
</span>
</span>
</p>

:::{.callout-tip icon="false" collapse="false"}
## {{< fa solid tree-city >}} &nbsp; Optional or of Interest

 * [Visualizing phylogenetic trees](https://nbisweden.github.io/workshop-data-visualization-r/2304/home_content.html#51_Phylogenetic_trees)
 * [Visualizing maps with ggmap and leaflet](https://nbisweden.github.io/workshop-data-visualization-r/2304/lab_ggmap.html)
:::

:::{.callout-tip icon="false" collapse="false"}
## {{< fa brands python >}} &nbsp; Python and data visualization with Polars

If you are using Python as your main language of preference, we have here tried to reproduce some of the examples from the R course with `Polars` in combination with other python libraries. You can take a look at the examples [here](topics/python_polars.html)
:::

:::{.callout-tip icon="false" collapse="true"}
## {{< fa tools >}} &nbsp; Solutions to Lab Exercises

We have compiled all the solutions for the different exercises in each of the sections [here](topics/lab_solutions.html)
:::


